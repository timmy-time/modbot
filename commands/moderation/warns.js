...
module.exports = {
    ...
    run: async (client, message, args, arguments) => {
      ...
      try {
        if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply(`You are missing permission of "MANAGE_MESSAGES" permissions.`);
        ...
        if(!reason) reason = 'Unspecified';
          if (db.has(`myUserWarns.${wUser.id}.${message.guild.id}.warns`) == false) {
            db.set(`myUserWarns.${wUser.id}.${message.guild.id}`, { userid: `${wUser.id}`, guild: `${message.guild.id}`, warns: 0, reasons: []})
          }
          db.add(`myUserWarns.${wUser.id}.${message.guild.id}.warns`, 1)
          db.push(`myUserWarns.${wUser.id}.${message.guild.id}.reasons`, reason)
