...
module.exports = {
    ...
    run: async (client, messageCreate, args, arguments) => {
      ...
      try {
        if(!messageCreate.member.hasPermission("MANAGE_messageCreateS")) return messageCreate.reply(`You are missing permission of "MANAGE_messageCreateS" permissions.`);
        let wUser = messageCreate.mentions.users.first()
        if(!wUser) return messageCreate.reply("Couldn't find the user.") ;
        //if(wUser.hasPermission("MANAGE_messageCreateS")) return messageCreate.reply("You cannot warn another Moderator");
        ...
        messageCreate.channel.send(successEmbed);
      }
      catch(err) {
